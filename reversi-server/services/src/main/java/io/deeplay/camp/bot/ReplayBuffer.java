...
/**
 * The {@code ReplayBuffer} class serves as a storage for experiences encountered in reinforcement learning.
 * It allows adding new experiences, updating their priorities, and sampling a batch of experiences
 * based on their priorities for training purposes. The priorities influence the probability of each
 * experience being sampled, using a variant of prioritized experience replay.
 *
 * <p>The replay buffer has a fixed capacity, and once it reaches maximum capacity, the oldest
 * experience will be removed to make room for new ones. This class implements an efficient experience
 * sampling mechanism, utilizing priority sampling with proportional priorities and adjusts
 * priority influence based on buffer size.
 *
 * <p>The buffer uses parameters `PRIORITY_ALPHA_BASE` and `PRIORITY_BETA_BASE` to control the extent to which
 * priorities are used when sampling experiences and adjusting their importance sampling weights, respectively.
 * These parameters change dynamically with buffer size to maintain sampling efficacy as the buffer scales.
 *
 * <p>Additionally, this class supports gradual forgetting of older experiences through importance sampling
 * weight adjustments, implemented by dynamically calculating a maximum importance weight for the sampled batch.
 *
 * <p><b>Usage:</b>
 * <pre>{@code
 * ReplayBuffer replayBuffer = new ReplayBuffer(5000);
 * replayBuffer.add(new Experience(state, action, reward, nextState, done));
 * List<Experience> sampleBatch = replayBuffer.sample(32);
 * }</pre>
 *
 * @see Experience
 */
